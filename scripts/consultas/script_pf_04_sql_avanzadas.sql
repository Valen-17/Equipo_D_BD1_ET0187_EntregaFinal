...
SELECT pro.id_proyecto AS codigo_proyecto,
       ...
       eva.puntaje AS puntaje_evaluacion,
       eva.comentario AS comentario_evaluacion,
...
ORDER BY pro.id_proyecto, eva.puntaje DESC;

-- Consulta 4: -- Consulta 4: Total de proyectos por programa y facultad
SELECT 
...
JOIN asignatura a ON a.id_programa = prog.id_programa
JOIN grupo g ON g.id_asignatura = a.id_asignatura
JOIN grupo_proyecto gp ON gp.id_curso = g.id_curso
JOIN proyecto pr ON pr.id_proyecto = gp.id_proyecto
GROUP BY facu.nombre, prog.nombre_programa
ORDER BY facu.nombre, prog.nombre_programa;
...
-- Consulta 9: Listado general de evaluadores con resultados de evaluación de los proyectos asignados
SELECT CONCAT(ev.nombre, ' ', ev.apellido) AS nombre_evaluador,
       COUNT(DISTINCT eva.id_proyecto) AS cantidad_proyectos_evaluados,
       AVG(eva.puntaje) AS puntaje_promedio,
       MAX(eva.puntaje) AS puntaje_maximo,
       MIN(eva.puntaje) AS puntaje_minimo,
       SUM(eva.puntaje) AS suma_puntajes
FROM evaluacion eva
-- Relacionamos evaluación con usuario (evaluador)
JOIN usuario usu ON eva.id_usuario = usu.id_usuario
JOIN evaluador ev ON usu.id_usuario = ev.id_usuario
-- Agrupamos por evaluador para calcular los resultados generales
GROUP BY ev.id_evaluador, ev.nombre, ev.apellido
ORDER BY nombre_evaluador;

-- Consulta 10: Listado detallado de evaluadores con resultados de evaluación de los proyectos asignados
SELECT CONCAT(ev.nombre, ' ', ev.apellido) AS nombre_evaluador,
       pro.id_proyecto AS codigo_proyecto,
       pro.titulo AS titulo_proyecto,
       tip.id_tipo_proyecto AS codigo_tipo_proyecto,
       tip.tipo_proyecto AS nombre_tipo_proyecto,
       eva.puntaje AS puntaje_otorgado,
       eva.comentario AS comentario_evaluacion
FROM evaluacion eva
-- Relacionamos evaluación con usuario (evaluador)
JOIN usuario usu ON eva.id_usuario = usu.id_usuario
JOIN evaluador ev ON usu.id_usuario = ev.id_usuario
-- Relacionamos con el proyecto
JOIN proyecto pro ON eva.id_proyecto = pro.id_proyecto
JOIN tipo_proyecto tip ON pro.id_tipo = tip.id_tipo_proyecto
-- Ordenamos para que el listado quede organizado por evaluador, luego proyecto
ORDER BY nombre_evaluador, pro.id_proyecto;


-- Consulta 11: Listado de usuarios del sistema con su rol y características
...
FROM usuario usu
-- Relacionamos usuario con proyecto y rol (porque el rol está asignado por proyecto)
...
-- Consulta 12: Consulta libre de la cantidad de proyectos evaluados por tipo de proyecto y programa
...
-- JOIN con proyecto
JOIN proyecto pro ON eva.id_proyecto = pro.id_proyecto
-- JOIN con tipo de proyecto
JOIN tipo_proyecto tip ON pro.id_tipo = tip.id_tipo_proyecto
-- JOIN con grupo_proyecto para conectar con asignaturas
JOIN grupo_proyecto grppr ON pro.id_proyecto = grppr.id_proyecto
JOIN grupo grp ON grppr.id_grupo = grp.id_grupo
JOIN asignatura asi ON grp.id_asignatura = asi.id_asignatura
-- JOIN con programa académico
JOIN programa_academico prog ON asi.id_programa = prog.id_programa
-- Agrupamos por tipo de proyecto y programa
GROUP BY tip.id_tipo_proyecto, tip.tipo_proyecto, prog.id_programa, prog.nombre_programa
-- Ordenamos por tipo de proyecto y nombre de programa
